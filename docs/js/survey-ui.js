function initSurveyUI() {
  ...
  function scrollToSubmit() {
    ...
    if (n < TOTAL) {
      ...
      setMsg("يرجى الإجابة على جميع الأسئلة (" + n + " من " + TOTAL + ")", "err");
      return false;
    }
  ...
  document.getElementById("btn-save")?.addEventListener("click", async function () {
    ...
    if (email && email.indexOf("@") === -1) {
      setMsg("البريد الإلكتروني غير صالح", "err");
      return;
    }
    ...
    try {
      if (!window.clinicSurveySubmit) {
        throw new Error("نظام الإرسال غير جاهز");
      }
      const data = await window.clinicSurveySubmit({
        doctor_name: doctor,
        clinic_name: clinic,
        phone: phone,
        email: email,
